import { useMediaQuery } from '@mui/material';
import React, { useState } from 'react';
import '../../../App.css';
import TextField from '@mui/material/TextField';
import AccountCircleIcon from '@mui/icons-material/AccountCircle';
import MenuListComposition from './DropDown';
import MoneyIcon from '@mui/icons-material/Money';
import MenuAd from './MenuAd';

function FirstPart() {
    return <MenuAd />;
}

function ScndPart() {
    const isMobile = useMediaQuery('(max-width:700px)');
    const [isLoggedIn, setIsLoggedIn] = useState(true);

    const handleLogout = () => {
        setIsLoggedIn(false);
    };

    return (
        <div className={`absolute bg-[#1F1F1F] top-0 border-b-2 border-[#3B3B3B] h-[20%] ${isMobile ? 'left-0 w-full' : 'left-[18%] w-[82%]'} bg-[#333333]`}>
            <div className={`font-bold text-lg absolute top-[50px] text-white ${isMobile ? 'left-[20%]' : 'left-[15%]'}`}>Dashboard</div>
            {isMobile && isLoggedIn && <MenuListComposition />}
            <TextField
                size="small"
                variant="filled"
                label="Rechercher un Zem"
                placeholder="Entrez son Identifiant"
                InputProps={{ style: { backgroundColor: 'pink', color: 'white' } }}
                className={`absolute ${isMobile ? 'left-[45%] top-0' : 'left-[50%] top-[30px]'}`}
            />
            <AccountCircleIcon
                fontSize="large"
                style={{ color: 'blue', cursor: 'pointer' }}
                className={`absolute top-[36px] ${isMobile ? 'left-[90%]' : 'left-[90%]'}`}
                onClick={handleLogout}
            />
        </div>
    );
}

function ThirdPart() {
    const isMobile = useMediaQuery('(max-width:700px)');
    return (
        <div className={`overflow-auto absolute top-[20%] bg-[#1F1F1F] h-[100%] ${isMobile ? 'w-full left-0' : 'w-[82%] left-[18%]'}`}>
            <div className={`border-r-2 border-[#3B3B3B] grid grid-cols-3 gap-10 p-7 ${isMobile ? 'w-full' : 'w-[65%]'}`}>
                {[1, 2, 3].map((item, index) => (
                    <div key={index} className="flex flex-col items-center w-[200px] border border-gray-300 rounded-lg p-4 mr-6">
                        <div className="text-[#FFDE59] text-[14px] font-semibold">Nombre de Client</div>
                        <div className="text-white text-[16px] flex items-center mt-3">
                            <span className="ml-2 text-lg text-white">265</span>
                        </div>
                        <button className="mt-3 bg-blue-500 text-white py-2 px-4 rounded-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-opacity-75 text-[14px] font-semibold">Transporteurs</button>
                    </div>
                ))}
            </div>

            <div className={`border-r-2 border-[#3B3B3B] grid grid-cols-2 gap-10 p-7 ${isMobile ? 'w-full' : 'w-[65%]'}`}>
                {[
                    { label: 'courses', value: '1000', detail: 'personne' },
                    { label: 'paiement', value: '10000', detail: 'Transporteur' },
                    { label: 'courses', value: '300', detail: 'colis' },
                    { label: 'paiement', value: '500', detail: 'stand' },
                    { label: 'courses', value: '1000', detail: 'personne' },
                    { label: 'paiement', value: '10000', detail: 'Transporteur' },
                ].map((item, index) => (
                    <div key={index} className="w-full h-[150px] rounded-2xl border-2 border-[#3B3B3B] p-2">
                        <div className="text-white text-[32px]">{item.value}</div>
                        <div className="text-[#8E8E8E] text-[16px]">{item.label}</div>
                        <div className="text-[#8E8E8E] text-[16px]">{item.detail}</div>
                        <div className="text-[#FFDE59] text-[80%] ml-[70%]">voir les détails</div>
                    </div>
                ))}
            </div>
        </div>
    );
}

function FourthPart() {
    return (
        <div className="overflow-auto absolute top-[20%] border-b-2 border-[#3B3B3B] bg-[#1F1F1F] h-[80%] w-[18%] right-[10%]">
            <div className="flex items-center justify-between p-2">
                <div className="text-[#FFDE59] text-[80%]">Courses</div>
                <div className="text-[#FFDE59] text-[80%] ml-[20%]">...</div>
            </div>
            {[1, 2, 3, 4].map((item, index) => (
                <div key={index} className="w-full h-[75px] rounded-2xl border-2 border-[#3B3B3B] p-2 flex flex-col justify-center mb-2">
                    <div className="flex justify-between items-center mt-0">
                        <div className="text-white text-[20px] flex items-center">
                            <MoneyIcon style={{ color: '#FFDE59', fontSize: '1.5rem' }} />
                            <span className="ml-2">165f</span>
                        </div>
                        <div className="text-[#FFDE59] text-[70%] text-green-500">valider</div>
                    </div>
                    <div className="flex justify-between items-center mt-0">
                        <div className="text-[#8E8E8E] text-[20px]">topka-kandi</div>
                        <div className="text-[#8E8E8E] text-[11px]">10/02/2000</div>
                    </div>
                </div>
            ))}
        </div>
    );
}

export default function Dashboard() {
    const isMobile = useMediaQuery('(max-width:700px)');

    return (
        <div className="bg-[#1F1F1F]">
            {!isMobile && <FirstPart />}
            <div className="flex flex-col w-full">
                <ScndPart />
                <div className="flex">
                    <ThirdPart />
                    <FourthPart />
                </div>
            </div>
        </div>
    );
}
