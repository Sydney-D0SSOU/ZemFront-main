...
function ThirdPart() {
    ...
    const [editedZem, setEditedZem] = useState({
        ...
        numero_de_plaque: '', // Champ ajouté
        ID_stand: '' // Champ ajouté
    });

    const handleSubmit = async (event) => {
        event.preventDefault();
        try {
            const response = await axios.post('', editedZem);
            ...
            setEditedZem({
                ...
                numero_de_plaque: '', // Réinitialisation du champ
                ID_stand: '' // Réinitialisation du champ
            });
        } catch (error) {
...
<div className={`overflow-auto absolute top-[20%] border-[#3B3B3B] border-b-2 bg-[#1F1F1F] h-[80%] ${isMobile ? 'w-full left-0' : 'w-[82%] left-[18%]'}`}>
    <h1 className="text-white text-2xl font-bold p-4">Modifier les informations d'un ZEM</h1>
    <form onSubmit={handleSubmit} className="p-4 grid gap-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <label htmlFor="numero_de_plaque" className="text-white">Numéro de plaque :</label>
            <input
                type="text"
                name="numero_de_plaque"
                id="numero_de_plaque"
                value={editedZem.numero_de_plaque}
                onChange={handleChange}
                className="border border-gray-400 rounded-md p-2"
                required
            />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <label htmlFor="ID_stand" className="text-white">ID Stand :</label>
            <input
                type="text"
                name="ID_stand"
                id="ID_stand"
                value={editedZem.ID_stand}
                onChange={handleChange}
                className="border border-gray-400 rounded-md p-2"
                required
            />
